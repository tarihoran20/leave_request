...
@RestController
@RequestMapping("api/approvalmanager")
public class RestApprovalManagerController {
    ...
    @Autowired
    public RestApprovalManagerController(StatusService statusService, EmployeeService employeeService,
    ...
    public List<ApprovalDTO> getAll(ApprovalDTO approvalDTO, String email) {
        ...
        if (userDetails.getAuthorities().stream().anyMatch(a -> a.getAuthority().equals("Manager"))) {
            ...
            if (tempra.isEmpty() == false) { // jika approved/rejected tidak ada maka tampil requested
                ...
                for (Integer a : statusService.getAllRequestIdByEmployeeId(employeeService.getIdByEmail(email))) {
                    ...
                    if (statusService.getAllRequestByRequestIdAndEmployeeId(a, employeeService.getIdByEmail(email))
                            .isEmpty() == false
                        ...
                        temp.remove(a);
                            ...
                            .getAllRequestByRequestIdAndEmployeeId(a, employeeService.getIdByEmail(email))
                            .isEmpty() == false
                            && statusService.getAllRejectedApprovedByRequestIdAndEmployeeId(a,
    ...
    public ResponseEntity<Object> insertRejected(@PathVariable(required = true) Integer request_id) {
        ...
        if (result1) {
            return new ResponseEntity<>("Response from Manager Submitted", HttpStatus.OK);
        } else {
            return ResponseEntity.badRequest().body("FAILED Response from Manager Submitted");
        }
    }
    ...
    public ResponseEntity<Object> insertApproved(@PathVariable(required = true) Integer request_id) {
        ...
        employee3.setId(employeeService.getEmployeeHrId()); // approver hr
        ...
        if (result1 && result2) {
            ...
            return ResponseEntity.badRequest().body("FAILED Response from Manager Backup Submitted");
        }
