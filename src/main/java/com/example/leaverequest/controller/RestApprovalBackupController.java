...
@RestController
@RequestMapping("api/approvalbackup")
public class RestApprovalBackupController {
    ...
    @Autowired
    public RestApprovalBackupController(StatusService statusService, EmployeeService employeeService,
    ...
    // for (Integer a :
    // statusService.getAllRequestIdByEmployeeId(employeeService.getIdByEmail(email)))
    // {
    // tempra.add(statusService.getAllRejectedApprovedByRequestIdAndEmployeeId(a,
    // employeeService.getIdByEmail(email)));
    // }

    // if (tempra.isEmpty() == false) { //jika approved/rejected tidak ada maka
    // tampil requested
    ...
    // if (statusService.getAllRequestByRequestIdAndEmployeeId(a,
    // employeeService.getIdByEmail(email)).isEmpty() == false &&
    // statusService.getAllRejectedApprovedByRequestIdAndEmployeeId(a,
    // employeeService.getIdByEmail(email)).isEmpty() == false) {
    // temp.remove(a);
    // }
    // else if(statusService.getAllRequestByRequestIdAndEmployeeId(a,
    // employeeService.getIdByEmail(email)).isEmpty() == false &&
    // statusService.getAllRejectedApprovedByRequestIdAndEmployeeId(a,
    // employeeService.getIdByEmail(email)).isEmpty() == true) {
    // temp.add(statusService.getAllRequestByRequestIdAndEmployeeId(a,
    // employeeService.getIdByEmail(email)));
    // }
    // }
    ...
    public List<ApprovalDTO> getAll(ApprovalDTO approvalDTO, String email) {
        ...
        if (userDetails.getAuthorities().stream().anyMatch(a -> a.getAuthority().equals("Employee"))) {
            ...
            for (Integer a : statusService
                    .getAllRequestIdByEmployeeId(employeeService.getIdByEmail(userDetails.getUsername()))) {
                tempra.add(statusService.getAllRejectedApprovedByRequestIdAndEmployeeId(a,
                        employeeService.getIdByEmail(userDetails.getUsername())));
            }
            ...
            if (tempra.isEmpty() == false) { // jika approved/rejected tidak ada maka tampil requested
                ...
                for (Integer a : statusService
                    ...
                    if (statusService
                            .getAllRequestByRequestIdAndEmployeeId(a,
                                    employeeService.getIdByEmail(userDetails.getUsername()))
                            .isEmpty() == false
                            && statusService
                                    .getAllRejectedApprovedByRequestIdAndEmployeeId(a,
                                            employeeService.getIdByEmail(userDetails.getUsername()))
                                    .isEmpty() == false) {
                        temp.remove(a);
                        ...
                        List<Request> list_r = statusService.getAllRequestByRequestIdAndEmployeeId(a,
                                employeeService.getIdByEmail(userDetails.getUsername()));
                        list_r.forEach((r) -> {
    ...
    public ResponseEntity<Object> insertRejected(@PathVariable(required = true) Integer request_id) {
        ...
        employee.setId(
                statusService.getEmployeeIdRequesterByStatusId(statusService.getStatusIdByRequestId(request_id))); // requester
        status.setEmployee_request_id(employee);

        employee2
                .setId(statusService.getEmployeeIdApproverByStatusId(statusService.getStatusIdByRequestId(request_id))); // approver
        ...
        if (result1) {
            return new ResponseEntity<>("Response for Employee Backup Submitted", HttpStatus.OK);
        } else {
            return ResponseEntity.badRequest().body("FAILED Response for Employee Backup Submitted");
        }
    }
    ...
    public ResponseEntity<Object> insertApproved(@PathVariable(required = true) Integer request_id) {
        ...
        // TO MANAGER = REQUESTED

        request.setId(request_id); // request_id
        status2.setRequest(request);

        status2.setDate(java.time.LocalDate.now()); // date

        employee.setId(
                statusService.getEmployeeIdRequesterByStatusId(statusService.getStatusIdByRequestId(request_id))); // requester
        status2.setEmployee_request_id(employee);

        employee3.setId(employeeService.getManagerIdByEmail(email)); // approver manager
        status2.setEmployee_id(employee3);

        status2.setStatus("Requested ..");

        result2 = statusService.save2(status2);
